...
class GameScenePVA(Scene):
    def __init__(self, screen, *args, **kwargs):
        super(GameScenePVA, self).__init__(screen, *args, **kwargs)
        ...
        for i in range(8):
            for j in range(8):
                ...
                self.add(
                    f"indicator_{i}{j}",
                    Button(
                        image_file="images/black_circle.png",
                        x=x,
                        y=y,
                        width=40,
                        height=40,
        ...
        for i in range(8):
            for j in range(8):
                ...
                self.add(
                    f"capture_{i}{j}",
                    Container(
                        image_file="images/black_ring.png",
                        x=x,
                        y=y,
                        width=69,
                        height=69,
        ...
        def select_piece():
            ...
            for move in moves:
                ...
                if self.game.board[col][row] is not None:
                    ...
                    indicator.on_click = (lambda r, c: lambda: self.move(piece, r, c))(
                        row, col
                    )
    ...
    def computer_move(self):
        piece, move = self.game.get_ai_move(self.model, False)
        row, col = move
        p_row, p_col = piece.pos
        self.game.move(piece, row, col)
        self.set_board(p_row, p_col, None)
